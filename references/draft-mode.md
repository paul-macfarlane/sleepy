...
**Hands off the tooling once the draft is live.** Do not edit, rebuild, or re-test any Sleepy script while status is `drafting`. If something is broken, say so, use the one-shot fallback, and fix it after the draft or between mocks. (Mock #5: six picks went to autopick while a poller was being written mid-draft.)
...
📋 Queue Jeanty, Henry, A.J. Brown now.
```
