...
Every scheduled run goes through `scripts/scheduled_run.sh "<task>" <logname>`, never an agent CLI directly. The wrapper sets PATH (launchd and cron start with almost none), invokes `scripts/run_agent.sh` from `$SLEEPY_HOME`, appends to `logs/<logname>.log`, and posts a Discord alert if the selected agent exits non-zero. The agent adapter supplies provider-specific headless and permission flags; unattended runs cannot pause for approval prompts. Codex runs with a workspace-write sandbox plus outbound network access, while Claude retains Sleepy's existing unattended permission mode. In either provider, keep scheduled work constrained to the public read-only Sleeper API, Discord notifications, and files under `~/sleepy/`.

Provider selection is `SLEEPY_AGENT_RUNNER`, then `SLEEPY_AGENT`, then `agent.provider` in `config.json`, then auto-detection. `auto` prefers Claude for backward compatibility and falls back to Codex. `SLEEPY_AGENT_MODEL` or `agent.model` optionally pins the scheduled model. For any other provider, supply an executable in `SLEEPY_AGENT_RUNNER`; it receives the complete prompt as its only argument and must run non-interactively.
