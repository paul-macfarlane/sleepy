...
Sleepy is designed to be shared: every user runs it under their own Claude or OpenAI account, so there is no shared backend or shared model API key. Before publishing:
...
3. **README** covers: prerequisites (Claude Code or OpenAI Codex, `jq`, `python3`, a Discord server they control), provider-specific skill directories, first-run onboarding, agent selection, the draft-day and scheduling workflows (launchd on macOS, cron on Linux, both via `scripts/scheduled_run.sh`), Sleeper API etiquette (15s live / 5s mock / 30s idle polling), usage-limit expectations, and the privacy note (all data stays on the user's machine; the Discord webhook is a secret).
